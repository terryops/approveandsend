...
**框里显示的是回复本身，源码要点一下才出来。** 在此之前框里装的是 Markdown：该强调的
那句话两边挂着 `**`、金额前面顶着 `- `，而一个正在判断这份回复对不对的人，得在脑子里
把它渲染一遍。这块屏幕是拿来读的——写是一天里发生几次的例外——所以它默认显示渲染后的
样子，源码留给开始编辑的那一次点击。渲染走的是发送时用来拼那封信的同一段代码，而不是
另写一份：对「客户会收到什么」这个问题，这块屏幕存在的意义就是只给一个答案。改动
标色和它同时成立时，标色优先——那是有人按下去的控件，而渲染只是没人在打字时这张卡片
本来的样子。
...
**来信按它被写出来的样子渲染。**带 HTML 正文的邮件会以 HTML 呈现：表格、列表、加粗，
以及地址还留在上面的链接。在此之前这张桌子只存一份被压平的文字——和模型读的是同一份
——所以一张账单到了审阅者眼前是一列散落的词，「点这里」到了眼前则无处可点。markup 在
上屏之前会按白名单重建：认识的标签、认识的属性、认识的样式属性，以及任何能把元素挪出
正常文档流的声明一律不留。纯文本邮件，以及 HTML 超过半兆的邮件，读起来和以前完全一样。

长信会先收起，好让回复一直留在旁边的屏幕上，下面有一个按钮把余下的部分打开——本来就
放得下的信则一个按钮都不给，因为一个什么也不做的控件就是一个挡路的控件。

**远程图片会加载，但由服务端去取。**直接从陌生人服务器上取一张图就是一张已读回执：
它告诉对方这个地址是活的、有人打开了这封信、什么时候打开的、从哪个 IP、用什么浏览器。
所以由服务器取一次，再从你自己的源发给你——对方能知道的只是「有东西取了一张图」，没有
referrer、没有指纹。这也是所有正经邮件客户端十年来的做法。`MAIL_REMOTE_IMAGES=false`
可以关掉；关掉之后什么都不取，信上会写明它一共要了几张。

代理只会去取这张桌子渲染过的信里出现的地址——它们是签过名的——并且拒绝内网地址。
邮件里写 `<img src="http://169.254.169.254/">` 就是在要求这张桌子把自己的云凭据念出来，
而且域名是先解析再判断的，免得 `internal.example.com` 指向 10.0.0.1 却被当成外网。

**截图会显示出来。**「我这边看到的是这样」是客服邮件里一整个门类，它以内嵌图片的形式
到达。这些图片不是远程的——它们随邮件一起附在你自己的信箱里——所以它们会渲染在发件人
放它们的位置上，同时也出现在信下方那一排缩略图里。PNG、JPEG、GIF 和 WebP，也就是
解码出来只有像素、别无他物的那几种格式。其余一律只提供下载，SVG 尤其如此：SVG 是一份
可以携带脚本的文档，把陌生人发来的 SVG 显示在你自己的源里，等于把审阅者的会话交到
他手上。
...
默认为空，此时邮件那一半关闭：不调模型、界面上不出现第二块面板。见
[docs/review-language.md](docs/review-language.md)。

**「本来就是这个语言」也是一个答案，而且会被记下来。** 一支用中文回中文、审阅的人也读
中文的桌子，此前每封回复下面都挂着一句「暂无 zh-CN 译文——跑一次队列来渲染」。跑了也
没用：模型被问过了，它回答这份草稿已经是目标语言，而这个回答无处存放，于是屏幕永远分不
清「不需要」和「还没做」，下一次编辑再问一遍——在一张答案永远不会变的桌子上，永远。所以
现在有三种状态而不是两种：有内容的记录是一份译文，没有记录是一个还没人问过的问题，
空记录是问过了、答案是无事可做。只有第一种会渲染译文，只有第二种会出现那句提示。翻译
服务报错不算回答，那种情况必须重新问——一个服务挂了和一份不需要翻的回复，这中间的区别
就是这条。

草稿上方的那几张背景资料卡也会被渲染，但渲染成**界面**语言而不是这里这个：卡片是这块
屏幕上的家具，不是邮件，所以一支自己读得懂来信的团队，仍然会拿到用自己语言写的查询结果。
交给模型的那段话不动，仍是数据源自己写下的原文。改了界面语言，卡片会重新渲染；一台从未
选过语言的机器拿到的是数据源自己写下的原文——后台任务没有浏览器可问。
...
这四条事实——他是谁、从什么时候起、在用哪个方案、退款之后实际留下来多少——现在直接
印在做决定的两块屏幕上：审阅页，以及这个发件人自己的页面。此前它们只住在 `/billing/<地址>`，
藏在一句话里的一个链接后面，远到没有人会在「要不要答应退款」的当口点进去，而不点进去
的代价全部落在那封回复上。留下来的那个数字是**扣掉退款之后**的，不是流水——流水正是
一张桌子把一个已经全额退过款的人当成大客户对待的原因。

它是渲染时现问 Stripe 的，不走 `contextSources`：上下文来源跑在起草任务里，所以它只
答得上已经过过队列的邮件，在一个既没有任务也没有任务队列的发件人页面上什么都说不出来，
而且默认是关的。两边都开着时，审阅页显示的是来源那张卡而不是这张——模型拿到的是那一份，
而一个正在核对回复对不对的人，需要看见这份回复是照着什么写出来的。查询挂在 `Suspense`
后面，因为这是三次八秒超时的第三方 HTTPS 调用，而这两块屏幕本身只是读一个本地文件：
直接等着的话，Stripe 一抽风就是审阅页八秒打不开。没配密钥的桌子不会渲染这张卡，连占位
都没有。

**查无此人会明说出来**，而不是干脆不显示。一张压根不出现的卡片，和一次从来没跑成的查询
在屏幕上长得一模一样，而后一种读法正是有人对着一个根本不存在的账户答应退款的方式。
